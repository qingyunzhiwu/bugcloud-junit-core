...
public class AddAnnotationOfControllerTestClass extends AbstractTestClassDecorator {
	...
	public CtClass createTestClass(Class<?> clazz) throws Exception {
		...
		EnumMemberValue randomPort = new EnumMemberValue(classFile.getConstPool());
		randomPort.setType("org.springframework.boot.test.context.SpringBootTest.WebEnvironment");
		randomPort.setValue("RANDOM_PORT");
		annRunWith.addMemberValue("webEnvironment",randomPort);
		attr.addAnnotation(annRunWith);
